/**
 * PSEO-002: Conteúdo editorial e geradores de FAQ extraídos de lib/programmatic.ts.
 *
 * Contém blocos de texto editorial por setor, FAQs dinâmicas e conteúdo
 * regionalizado para páginas programáticas pSEO.
 */
import { formatBRL, getUfPrep, UF_REGION, REGION_NAMES, type Region } from './types';
...
export function getCidadeSectorEditorial(
  ...
  const paragraphs: Record<Region, string[]> = {
    ...
    nordeste: [
      `O mercado de licitações de ${sectorLower} em ${cityName} está em crescimento acelerado, impulsionado por investimentos federais e estaduais ${getUfPrep(uf)} ${ufName}. A região Nordeste vive uma transformação nas compras públicas, com crescente adoção de pregão eletrônico e maior transparência nos processos licitatórios, o que amplia as oportunidades para fornecedores qualificados de ${sectorLower}.`,
    ...
    centro_oeste: [
      `${cityName} é um polo estratégico para licitações de ${sectorLower} ${getUfPrep(uf)} ${ufName}, beneficiado pela concentração de órgãos federais e pela pujança econômica da região Centro-Oeste. A infraestrutura rodoviária interligando as capitais do Centro-Oeste confere vantagens logísticas a fornecedores de ${sectorLower} que atendem contratos com entrega em múltiplos pontos.`,
