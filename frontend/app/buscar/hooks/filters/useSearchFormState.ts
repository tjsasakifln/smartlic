...
export function useSearchFormState(clearResult: () => void): UseSearchFormStateReturn {
  ...
  const [ordenacao, setOrdenacao] = useState<OrdenacaoOption>(() => {
    if (typeof window === "undefined") return "confianca";
    const saved = safeGetItem("smartlic_sort_preference");
    if (saved && ["data_desc", "data_asc", "valor_desc", "valor_asc", "prazo_asc", "relevancia", "confianca"].includes(saved)) {
      return saved as OrdenacaoOption;
    }
    return "confianca";
  });
  ...
  // #1430 (VIAB-UX-001): Persist sort preference in localStorage
  useEffect(() => { safeSetItem("smartlic_sort_preference", ordenacao); }, [ordenacao]);
