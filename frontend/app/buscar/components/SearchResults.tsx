    ...
    <>
      ...
      {!loading && result && (result.resumo?.total_oportunidades ?? 0) === 0 && result.sources_degraded && result.sources_degraded.length > 0 && result.response_state !== "empty_failure" && (
        <div className="mt-6 text-center py-10" data-testid="degraded-zero-results">
          ...
          <div className="mt-6">
            ...
            <p className="mt-3 text-base font-medium text-ink">A fonte principal de dados está temporariamente indisponivel.</p>
      ...
      {!loading && result && (result.resumo?.total_oportunidades ?? 0) > 0 && (
        <div className={`mt-6 sm:mt-8 space-y-4 sm:space-y-6 ${!showGrid ? 'animate-fade-in-up' : ''}`}>
          ...
          <ResultsList result={result} paginatedLicitacoes={paginatedLicitacoes} totalLicitacoes={totalLicitacoes} currentPage={currentPage} pageSize={pageSize} onPageChange={setCurrentPage} onPageSizeChange={setPageSize} searchMode={searchMode} termosArray={termosArray} planInfo={planInfo} session={session} onShowUpgradeModal={onShowUpgradeModal} searchId={searchId} setorId={setorId} paywallApplied={paywallApplied} totalBeforePaywall={totalBeforePaywall} onTrackEvent={onTrackEvent} />
