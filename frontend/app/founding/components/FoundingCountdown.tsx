...
/**
 * BIZ-FOUND-002: deadline countdown + seat counter for the /founding landing.
 *
...
export default function FoundingCountdown({ snapshot }: Props) {
  ...
  if (!snapshot) {
    return (
      <div
        data-testid="founding-availability-loading"
  ...
  return (
    <div
      data-testid="founding-availability"
      className={`rounded-lg border p-4 ${counterColor}`}
      aria-live="polite"
    >
      <div className="flex items-baseline justify-between gap-2">
        <p className="text-sm font-medium" data-testid="founding-seat-counter">
          {full ? (
            <span>
            ...
            <span>
              <strong data-testid="founding-seats-remaining">{seatsRemaining}</strong>
              /{seatsTotal} vagas restantes
              {urgency && ' — corra!'}
            </span>
          )}
        </p>
        <p className="text-xs uppercase tracking-wide opacity-80" data-testid="founding-discount-label">
          {snapshot.discount_pct}% off vitalício
        </p>
      </div>
      ...
      {snapshot.deadline_at && countdown.total_ms > 0 && (
        <div
          ...
          data-testid="founding-countdown"
        >
          <span className="opacity-70">Encerra em:</span>
          <div className="flex items-center gap-2 font-mono tabular-nums">
            <span data-testid="founding-countdown-days">
              <strong>{countdown.days}</strong>d
            </span>
            <span data-testid="founding-countdown-hours">
              <strong>{pad2(countdown.hours)}</strong>h
            </span>
            <span data-testid="founding-countdown-minutes">
              <strong>{pad2(countdown.minutes)}</strong>m
            </span>
            <span data-testid="founding-countdown-seconds">
      ...
      {snapshot.deadline_at && countdown.total_ms === 0 && (
        <p className="mt-2 text-sm" data-testid="founding-countdown-expired">
          O prazo de inscrição founding terminou em 30/05/2026.
        </p>
      )}

      {snapshot.paused && (
        <p className="mt-2 text-sm" data-testid="founding-paused-notice">
