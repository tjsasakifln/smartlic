...
async def _fetch_mrr(sb, period_start: str, period_end: str) -> tuple[float, int, list[MrrHistoryEntry]]:
    """Call get_mrr(start_date, end_date) and return (mrr, subscriber_count, history)
    for all months in the range. History is ordered chronologically."""
    res = await sb_execute(
        sb.rpc("get_mrr", {"start_date": period_start, "end_date": period_end}),
        category="rpc",
    )
    rows = res.data or []
    history: list[MrrHistoryEntry] = []
    mrr_val = 0.0
    sub_count = 0
    for row in rows:
        month = str(row.get("month", ""))
        m = float(row.get("mrr", 0))
        s = int(row.get("subscriber_count", 0))
        history.append(MrrHistoryEntry(month=month, mrr=m, subscriber_count=s))
        mrr_val = m
        sub_count = s
    return mrr_val, sub_count, history
