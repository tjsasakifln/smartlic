...
async def sitemap_cnpjs(response: Response):
    ...
    # Alert: empty despite data exists in source table
        ...
        try:
            ...
            if mv_count and mv_count > 0:
                sentry_sdk.capture_message('sitemap_empty_despite_data', level='error',
                    tags={'endpoint': 'cnpjs', 'source_count': mv_count})
        except Exception:
            pass

    # Alert 3: stale data - check max data_publicacao in source table
    try:
        from supabase_client import get_supabase, sb_execute
        sb = get_supabase()
        resp = await sb_execute(
        ...
        if resp.data and len(resp.data) > 0:
            ...
            if raw:
                last_date = (str(raw)[:10] if isinstance(raw, str) else str(raw)[:10])
                last_dt = datetime.fromisoformat(last_date).replace(tzinfo=timezone.utc) if 'T' not in last_date else datetime.fromisoformat(last_date)
...
async def sitemap_fornecedores_cnpj(response: Response):
    ...
    fornecedor_list = data.get("cnpjs", [])
    if len(fornecedor_list) == 0:
        try:
            from supabase_client import get_supabase, sb_execute
            sb = get_supabase()
            resp = await sb_execute(
                sb.table("pncp_supplier_contracts").select("ni_fornecedor", count="exact").neq("ni_fornecedor", "").not_.is_("ni_fornecedor", "null").limit(0)
            )
            mv_count = resp.count if hasattr(resp, 'count') and resp.count is not None else 0
            if mv_count and mv_count > 0:
                sentry_sdk.capture_message('sitemap_empty_despite_data', level='error',
                    tags={'endpoint': 'fornecedores-cnpj', 'source_count': mv_count})
        except Exception:
            pass

    # Alert 3: stale data - check max data_assinatura in source table
    try:
        from supabase_client import get_supabase, sb_execute
        sb = get_supabase()
        resp = await sb_execute(
    ...
    record_sitemap_count("fornecedores-cnpj", len(fornecedor_list))
