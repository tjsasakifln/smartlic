...
from schemas.widget_compint import (
    FornecedorShare,
    MarketShareData,
    MesSerie,
    MonthlyTrendData,
...
async def _get_cached(key: str) -> Optional[dict]:
    ...
    try:
        ...
        logger.debug("widget_compint: Redis cache read failed: %s", e)
...
async def _set_cached(key: str, data: dict) -> None:
    ...
    try:
        ...
        logger.debug("widget_compint: Redis cache write failed: %s", e)
...
def _compute_hhi(fornecedores: list[dict], total_value: float) -> str:
    ...
    HHI < 1500 = Baixa, 1500-2500 = Média, > 2500 = Alta.
    """
...
def _build_market_share(rpc_data: dict) -> MarketShareData:
...
def _build_top_winners(rpc_data: dict) -> TopWinnersData:
...
def _build_orgao_ranking(rpc_data: dict) -> OrgaoRankingData:
...
async def _check_rate_limit(request: Request) -> None:
    ...
    try:
        ...
        logger.debug("widget_compint: rate limit check failed (fail-open): %s", e)
...
@router.get(
    ...
    description=(
        "Retorna dados agregados de contratos públicos por setor para widget embedável.\n\n"
        "Temas:\n"
...
async def widget_competitive_intel(
        ...
        return _build_cors_response(
            {
                "error": "invalid_setor",
                "message": (
                    f"Setor inválido: '{setor}'. "
                    f"Setores válidos: {', '.join(valid_sectors)}"
                ),
            },
            status_code=400,
        )
    ...
    # ── Cache key
    cache_key = f"{setor}:{uf_clean or 'BR'}:{tema_lower}"
    ...
    try:
        ...
        logger.error("widget_compint: RPC failed for setor=%s uf=%s: %s", setor, uf_clean, e)
        return _build_cors_response(
            {
                "error": "data_unavailable",
                "message": "Dados temporariamente indisponíveis. Tente novamente em instantes.",
            },
            status_code=503,
        )
    ...
    period_label = f"Últimos {_WINDOW_MONTHS_DEFAULT} meses"

    if tema_lower == "market-share":
        dados = _build_market_share(rpc_data)
    elif tema_lower == "top-winners":
        dados = _build_top_winners(rpc_data)
    elif tema_lower == "monthly-trend":
        dados = _build_monthly_trend(rpc_data)
    elif tema_lower == "orgao-ranking":
        dados = _build_orgao_ranking(rpc_data)
    else:
        return _build_cors_response(
            {"error": "invalid_tema"},
            status_code=400,
