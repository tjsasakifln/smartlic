...
Routes:
  - GET /v1/subcontract/health              SUBINTEL-030: Health/feature gate check
...
from quota.plan_auth import check_subcontract_intel_access, get_subcontract_intel_dependency
...
# Router for gated subcontract endpoints (requires intel capability)
router = APIRouter(
    tags=["subcontract"],
...
# ============================================================================
# SUBINTEL-030 (#1665): Health / Feature Gate endpoint
# ============================================================================


@health_router.get(
    "/subcontract/health",
    summary="Subcontract Intel Health (SUBINTEL-030)",
)
async def subcontract_health(user: dict = Depends(require_auth)):
    """Return feature flag status and user access for subcontract intel.

    This endpoint is NOT gated by the subcontract intel capability — it is
    publicly accessible (authenticated) so clients can check if the feature
    is available before attempting gated endpoints.
    """
    enabled = get_feature_flag("SUBCONTRACT_INTEL_ENABLED")
    has_access = await check_subcontract_intel_access(user)
    return JSONResponse(
        content={
            "enabled": enabled,
            "has_access": has_access,
            "feature_flag": "SUBCONTRACT_INTEL_ENABLED",
        }
    )
