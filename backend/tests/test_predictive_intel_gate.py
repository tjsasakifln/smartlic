"""Tests for the Predictive Intelligence gate (PREDINT-000, EPIC-PREDINT #1260).

Covers:
  - Non-regression: every existing plan defaults allow_predictive_intel=False
...
from quota import PLAN_CAPABILITIES, requires_predictive_intel
...
class TestNonRegression:
    ...
    def test_existing_plan_defaults_predictive_intel_false(self, plan_id):
        assert plan_id in PLAN_CAPABILITIES
        assert PLAN_CAPABILITIES[plan_id]["allow_predictive_intel"] is False
...
def _mock_quota(allow: bool, plan_id: str = "free_trial"):
    return SimpleNamespace(
        capabilities={"allow_predictive_intel": allow},
        plan_id=plan_id,
        allowed=True,
    )
...
class TestPredictiveIntelGate:
    async def test_flag_off_returns_404(self):
        ...
        with patch("config.features.get_feature_flag", return_value=False):
            with pytest.raises(HTTPException) as exc:
                await requires_predictive_intel(USER)
    ...
    async def test_flag_on_capability_false_returns_403(self):
        ...
        assert exc.value.detail["error_code"] == "predictive_intel_not_available"
    ...
    async def test_flag_on_capability_true_passes(self):
        ...
        with patch("config.features.get_feature_flag", return_value=True), \
            ...
            result = await requires_predictive_intel(USER)
        assert result == USER
