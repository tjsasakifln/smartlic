"""Tests for the B2G Operations gate (B2GOPS-000, EPIC-B2GOPS #1262).

Covers:
  - Non-regression: every existing plan defaults allow_workspace_basic=False
...
from quota import PLAN_CAPABILITIES, requires_workspace_basic
...
class TestNonRegression:
    ...
    def test_existing_plan_defaults_workspace_basic_false(self, plan_id):
        assert plan_id in PLAN_CAPABILITIES
        assert PLAN_CAPABILITIES[plan_id]["allow_workspace_basic"] is False
...
def _mock_quota(allow: bool, plan_id: str = "free_trial"):
    return SimpleNamespace(
        capabilities={"allow_workspace_basic": allow},
        plan_id=plan_id,
        allowed=True,
    )
...
class TestB2GOpsGate:
    async def test_flag_off_returns_404(self):
        ...
        with patch("config.features.get_feature_flag", return_value=False):
            with pytest.raises(HTTPException) as exc:
                await requires_workspace_basic(USER)
    ...
    async def test_flag_on_capability_false_returns_403(self):
        ...
        assert exc.value.detail["error_code"] == "b2g_ops_not_available"
    ...
    async def test_flag_on_capability_true_passes(self):
        ...
        with patch("config.features.get_feature_flag", return_value=True), \
            ...
            result = await requires_workspace_basic(USER)
        assert result == USER
