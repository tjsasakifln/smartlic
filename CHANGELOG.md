...
- **Re-enable E2E tests no CI com sharding 4×4 + fix Mobile Safari (#2057)** — 62 cancelamentos consecutivos resolvidos. Causas: (1) `timeout-minutes` insuficiente em todos os jobs (15→30 no backend/e2e, 20→35 no critical-flows); (2) Projeto `Mobile Safari` (iPhone 13) no `playwright.config.ts` executava 453 testes extras no CI — excluído condicionalmente (`CI=true` → skip) + `--project=chromium` nos comandos; (3) Health check URL errada (`/health`→`/health/live`). E2E sharding 4×4 (4 workers × 4 matrix shards = 16-way parallelism, ~15 min). `tests.yml` E2E job validado via `workflow_dispatch` antes do merge. Rollback: reverter PR #2057.
